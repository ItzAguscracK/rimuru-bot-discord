...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if(message.author == owner){
      ...
      if (user.roles.cache.has(role.id)) return message.channel.send("El usuario ya tiene el rol mencionado")

      await user.roles.add(role.id)
      return message.channel.send(`El rol ${role.name} ha sido agregado a el usuario ${user}`)
    }
